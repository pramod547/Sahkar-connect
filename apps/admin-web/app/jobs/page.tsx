  ...
  return (
    <div className="p-8 max-w-7xl mx-auto w-full">
      <header className="mb-8 pb-4 border-b border-gray-200">
        <h1 className="text-2xl font-bold text-[#0B4F4A]">Society Live Jobs & Fair-Match Audit</h1>
          ...
          <tbody className="divide-y divide-gray-200">
            {jobs.map((j) => (
              <tr key={j.id} className="hover:bg-gray-50 transition">
                ...
                <td className="p-4">
                  <span className="bg-[#147D74]/10 text-[#147D74] text-xs font-bold px-2.5 py-1 rounded-full uppercase">
                    {j.status}
                  </span>
                </td>
                <td className="p-4 font-semibold text-[#0B4F4A]">{j.assignedWorker}</td>
                <td className="p-4 font-bold text-[#E8A33D]">{j.fairMatchScore.toFixed(4)}</td>
                <td className="p-4 text-right">
                  ...
                  >
                    <Sparkles className="w-3.5 h-3.5 mr-1 text-[#E8A33D]" /> Audit Match
                  </button>
                </td>
              </tr>
            ))}
      ...
      {selectedJobExplain && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 max-w-xl w-full shadow-2xl border border-gray-200">
            <div className="flex justify-between items-start mb-4 border-b pb-3">
              <div>
                <span className="text-xs font-bold text-[#E8A33D] uppercase tracking-wider">
                  Fair-Match Explainability Audit
                </span>
                <h3 className="text-lg font-bold text-[#0B4F4A]">
            ...
            <p className="text-xs text-gray-600 mb-4">
              Selected Worker: <strong className="text-[#0B4F4A]">{selectedJobExplain.assignedWorker}</strong> (Final Score: {selectedJobExplain.fairMatchScore})
            </p>
            ...
            <div className="space-y-3">
              <div>
                <div className="flex justify-between text-xs font-medium mb-1">
                  ...
                  <span className="font-bold">{(selectedJobExplain.breakdown.proximity * 100).toFixed(0)}%</span>
                </div>
              ...
              <div>
                <div className="flex justify-between text-xs font-medium mb-1">
                  <span>Rotation Fairness Score (W_fair = 35%)</span>
                  <span className="font-bold text-[#2E8B57]">{(selectedJobExplain.breakdown.fairness * 100).toFixed(0)}%</span>
                </div>
                <div className="w-full bg-gray-100 h-2.5 rounded-full overflow-hidden">
                  <div
              ...
              <div>
                <div className="flex justify-between text-xs font-medium mb-1">
                  ...
                  <span className="font-bold">{(selectedJobExplain.breakdown.rating * 100).toFixed(0)}%</span>
                </div>
                <div className="w-full bg-gray-100 h-2.5 rounded-full overflow-hidden">
                  <div
              ...
              <div>
                <div className="flex justify-between text-xs font-medium mb-1">
                  ...
                  <span className="font-bold">{(selectedJobExplain.breakdown.skill * 100).toFixed(0)}%</span>
                </div>
                <div className="w-full bg-gray-100 h-2.5 rounded-full overflow-hidden">
                  <div
            ...
            <div className="mt-6 flex justify-end">
              <button
                onClick={() => setSelectedJobExplain(null)}
                className="bg-[#0B4F4A] text-white text-xs font-bold px-4 py-2 rounded-lg"
              >
                Close Audit Panel
              </button>
            </div>
